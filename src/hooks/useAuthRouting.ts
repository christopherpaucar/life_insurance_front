...
export function useAuthRouting() {
  ...
  const userRole = useMemo(() => {
    return user?.roles?.[0]?.name as RoleType | undefined;
  }, [user?.roles]);

  // Get all user roles as array
  const userRoles = useMemo(() => {
    return user?.roles?.map((r) => r.name as RoleType) || [];
  }, [user?.roles]);

  // Check if user has access to current route
  const canAccessCurrentRoute = useMemo(() => {
    if (!hydrated || !isAuthenticated) return false;
    return routingService.canUserAccessRoute(pathname, userRoles);
  }, [hydrated, isAuthenticated, pathname, userRoles, routingService]);
