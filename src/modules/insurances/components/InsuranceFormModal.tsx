  ...
  const handleFrequencyToggle = (frequency: PaymentFrequency) => {
    setFormData((prev) => {
      ...
      if (currentFrequencies.includes(frequency)) {
        return {
          ...prev,
          availablePaymentFrequencies: currentFrequencies.filter((f: PaymentFrequency) => f !== frequency),
        }
      } else {
        return {
          ...prev,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        if (freqChanged) updateDto.availablePaymentFrequencies = formData.availablePaymentFrequencies
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isProcessing}>
              {isProcessing ? 'Guardando...' : mode === 'create' ? 'Guardar Plan' : 'Guardar Cambios'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
