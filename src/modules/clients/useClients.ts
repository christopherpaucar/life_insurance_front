...
export const useClients = (params?: ClientQueryParams) => {
  ...
  const updateClientMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateClientDto }) => clientsService.updateClient(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: CLIENT_QUERY_KEYS.all })
  ...
  const linkUserAccountMutation = useMutation({
    mutationFn: ({ id, userId }: { id: string; userId: string }) => clientsService.linkUserAccount(id, userId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: CLIENT_QUERY_KEYS.all })
