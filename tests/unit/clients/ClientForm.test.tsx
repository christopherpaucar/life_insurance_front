...
describe('ClientFormModal', () => {
  ...
  it('debe renderizar el formulario en modo creación', () => {
    ...
    expect(
      screen.getByText('Complete el formulario para crear un nuevo cliente')
    ).toBeInTheDocument()
  ...
  it('debe renderizar el formulario en modo edición con los datos del cliente', () => {
    ...
    expect(screen.getByLabelText('Número de Documento')).toHaveValue(
      mockClient.identificationNumber
    )
  ...
  it('debe validar los campos requeridos al crear un cliente', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('El número de documento debe tener al menos 5 caracteres')
      ).toBeInTheDocument()
